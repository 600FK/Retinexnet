...
class lowlight_enhance(object):
    ...
    def train(self, train_low_data, train_high_data, eval_low_data, batch_size, patch_size, epoch, lr, sample_dir, ckpt_dir, eval_every_epoch, train_phase):
        ...
        for epoch in range(start_epoch, epoch):
            for batch_id in range(start_step, numBatch):
                ...
                for patch_id in range(batch_size):
                    ...
                    # data = np.array(data_augmentation(train_low_data[image_id][x : x+patch_size, y : y+patch_size, :], rand_mode))
                    # print(data.shape)
                    # data_1 = np.array(batch_input_low[patch_id, :, :, :])
                    # print(data_1.shape)
                    # data_2 = np.array(batch_input_high[patch_id, :, :, :])
                    # print(data_2.shape)
        ...
        # 在训练结束后保存损失历史并绘制损失曲线
        loss_history_plot_path = os.path.join(sample_dir, 'loss_history.png')
        ...
        # 保存损失历史曲线图像
        plt.savefig(loss_history_plot_path)

        # 显示损失历史曲线图像
        plt.show()

        print(f"Loss history plot saved at: {loss_history_plot_path}")
